...
class BulkInsertTable:
    ...
    def __init__(
        self, table_name, select_conn, insert_conn=None, batch_size=None, unlogged=None
    ):
        """
        This init function expects to be passed a database connection to read data from
        and an optional DB connection to write data to. If the insert_conn is
    ...
    def get_index_names(self):
        ...
        Returns a list of of tuples of (index names, column defintion, unique) strings:
            [("cache_ndx_company_name", "name", True)]
        Will create the unique index cache_ndx_company_name on name.
        """
        return []
    ...
    def process_row(self, row):
        ...
        dict as follows instead:
            { "cache.company": [ ... row data ... ],
              "cache.canonical_company": [ ... row data ... ] }
    ...
    def _create_tables(self):
        ...
        try:
            with conn.cursor() as curs:
                ...
                curs.execute(
                    f"CREATE {'UNLOGGED' if self.unlogged else ''} TABLE {self.temp_table_name} ({columns})"
                )
                conn.commit()
        ...
        try:
            with conn.cursor() as curs:
                for name, column_def, unique in self.get_index_names():
                    ...
                    curs.execute(
                        f"CREATE {uniq} INDEX {name}_tmp ON {self.temp_table_name} {column_def}"
                    )
                ...
                for name, types in self.get_create_table_columns():
                    if name == "id" and types == "SERIAL":
                        ...
                        curs.execute(
                            f"""SELECT setval('{self.temp_table_name}_id_seq', max(id) + 1, false)
    ...
    def swap_into_production(self, no_swap_transaction=False, swap_conn=None):
        ...
        try:
            with conn.cursor() as curs:
                ...
                curs.execute(
                    f"ALTER TABLE {self.temp_table_name} RENAME TO {simple_table_name}"
                )
                for name, column_def, _ in self.get_index_names():
                    if schema:
                        curs.execute(
                            f"ALTER INDEX {schema}.{name}_tmp RENAME TO {name}"
                        )
                    else:
                        curs.execute(f"ALTER INDEX {name}_tmp RENAME TO {name}")
                curs.execute(
                    f"ALTER SEQUENCE IF EXISTS {self.temp_table_name}_id_seq RENAME TO {simple_table_name}_id_seq"
                )
    ...
    def run(self, no_swap=False, no_analyze=False):
        """
        The main loop of the class that carefully executes the provided statements
        ...
        with self.insert_conn.cursor(
            ...
            for i, db_query_vals in enumerate(zip_longest(queries, values)):
                    ...
                    while True:
                        ...
                        if len(rows) >= self.batch_size:
                            ...
                            if batch_count % 20 == 0:
                                ...
                                log(
                                    f"{self.table_name}: inserted {inserted:,} from {row_count:,} rows. {percent}% complete"
                                )
                ...
                if rows:
                    insert_rows(
                        ins_curs, self.temp_table_name, rows, cols=self.insert_columns
                    )
                    self.insert_conn.commit()
